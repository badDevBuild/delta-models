"""Root-owned G17 and QSZ92G miniature exterior helpers, frozen independently."""
...
def build_pistol(asset,length,kind):
 assert kind in {'g17','qsz'}, 'Only the two root-owned exterior sculptures'
 ...
 for sign in [-1,1]:
  ...
  for j in range(13):
   z=5-j*2.1
   cx=(23+max(0,-z)*.30) if kind=='beretta' else ((19+max(0,-z)*.31) if kind=='g17' else (21+max(0,-z)*.10))
   for i in range(5):
